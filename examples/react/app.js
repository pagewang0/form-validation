...
var item = (name, doc, id, handle_change, handle_blur) => React.createElement(
    "div",
    { className: doc.parent, key: id },
    React.createElement(
        "label",
    ...
    React.createElement(
        "div",
        { className: "col-sm-10" },
        React.createElement("input", { type: doc.type ? doc.type : 'text', className: doc.style, id: name, name: name,
            value: doc.value,
            onChange: handle_change.bind(_this, name),
            onBlur: doc.blur ? handle_blur.bind(_this) : null
        }),
        React.createElement(
            "div",
...
var validation = (state, setState) => {
    ...
    validator.path('username').validate(function (field, username, done) {
        request('/username').query({ username: username }).end((err, res) => {
            if (err) {
                done(err, field);
                return;
            };
    ...
    validator.path('phone').validate(function (field, phone, done) {
        request('/phone').query({ phone: phone }).end((err, res) => {
            if (err) {
                done(err, field);
                return;
            };
    ...
    validator.submit(function () {
        ...
        request.post('/user').send({ user: user }).end((err, res) => {
            if (err) {
                console.log(err);
                return;
            };
            ...
            React.createElement(
                ...
                React.createElement(
                    ...
                    React.createElement(
                        ...
                        React.createElement(
                            "button",
                            { type: "submit", className: "btn btn-primary pull-right", id: "submit", "data-code": state.code },
                            "Submit"
                        )
                    )
                )
